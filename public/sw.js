// =============================================================
// SERVICE WORKER — La Rose Smart Waste v2.0
// Cache offline-first para funcionamento sem internet.
// Para forçar atualização em todos os dispositivos:
//   mude o número de CACHE_NAME abaixo e faça deploy.
// =============================================================
const CACHE_NAME = 'larose-v2.0';

// Lista de arquivos para cache offline
// Adicione aqui qualquer novo arquivo que precisar funcionar offline
const ASSETS_TO_CACHE = [
    '/',
    '/index.html',
    '/lancamento.html',
    '/dashboard.html',
    '/manifest.json',
    '/css/style.css?v=2.0',
    '/js/modal.js?v=2.0',
    '/js/app.js?v=2.0',
    '/js/data.js',
    '/js/firebase-config.js',
    '/assets/images/logo.png',
    '/assets/icons/icon-192.png',
    '/assets/icons/icon-512.png',
    // Fontes
    'https://fonts.googleapis.com/css2?family=Roboto:wght@400;500;700;900&family=Inter:wght@300;400;500;600;700;800;900&display=swap',
    // Biblioteca Excel
    'https://cdnjs.cloudflare.com/ajax/libs/xlsx/0.18.5/xlsx.full.min.js'
];

// Instalação: pré-cache de todos os assets
self.addEventListener('install', (event) => {
    self.skipWaiting(); // Ativa o SW imediatamente, sem esperar aba fechar
    event.waitUntil(
        caches.open(CACHE_NAME).then(c => c.addAll(ASSETS_TO_CACHE))
    );
});

// Ativação: remove caches antigos e notifica os clientes
self.addEventListener('activate', (event) => {
    event.waitUntil(
        caches.keys()
            .then(names => Promise.all(
                names.map(n => n !== CACHE_NAME && caches.delete(n))
            ))
            .then(() => self.clients.claim())
            .then(() => self.clients.matchAll({ type: 'window', includeUncontrolled: true }))
            .then(clients => clients.forEach(c => c.postMessage({ type: 'SW_ATUALIZADO' })))
    );
});

// Interceptação de requisições: cache-first com fallback para rede
self.addEventListener('fetch', (event) => {
    const req = event.request;
    if (req.method !== 'GET') return; // Ignora POST, PUT, etc.

    // HTML: rede primeiro (sempre pega versão mais recente), fallback para cache
    if (req.headers.get('accept')?.includes('text/html')) {
        event.respondWith(
            fetch(req)
                .then(r => { caches.open(CACHE_NAME).then(c => c.put(req, r.clone())); return r; })
                .catch(() => caches.match(req).then(c => c || caches.match('/index.html')))
        );
        return;
    }

    // Outros assets: cache primeiro, depois rede
    event.respondWith(
        caches.match(req).then(c => c || fetch(req).then(r => {
            caches.open(CACHE_NAME).then(cache => cache.put(req, r.clone()));
            return r;
        }))
    );
});
